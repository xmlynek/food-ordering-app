  ...
  public Mono<Page<BasicRestaurantDto>> findAllRestaurants(Pageable pageable, String searchValue) {
    var criteria = QueryBuilders.bool()
        ...
        .filter(QueryBuilders.bool()
            .should(QueryBuilders.match().field("name").query(searchValue).analyzer("standard")
                .build()
                ._toQuery())
            .should(QueryBuilders.match().field("description").query(searchValue).analyzer("standard")
                .build()
                ._toQuery())
