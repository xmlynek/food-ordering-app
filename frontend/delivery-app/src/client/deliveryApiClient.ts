...
export const fetchCourierDeliveryHistory = async (page: number = 0, size: number = 10): Promise<PageableRestApiResponse<Delivery>> => {
  const response = await axiosInstance.get(`${window.envVars.REACT_DELIVERY_SERVICE_PATH}/history?page=${page}&size=${size}`);
  return response.data;
};
